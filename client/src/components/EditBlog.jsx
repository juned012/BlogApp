...
const EditBlog = ({ onClose, blog }) => {
  const { handleEditBlogPost } = useContext(UserContext);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await handleEditBlogPost(blog._id, updatedData);
      onClose(); // This will also re-fetch in the parent
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-lg p-8 max-w-4xl w-full relative space-y-4">
        <div className="overflow-y-auto max-h-96">
          <h1 className="text-center text-2xl font-bold mb-8">Edit Blog</h1>
